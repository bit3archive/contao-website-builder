    ...
    extends BackendModule
{
            ...
            if ($this->Input->post('FORM_SUBMIT') == 'tl_multilang_create') {
                ...
                $objPage = $this->Database
                    ...
                    "SELECT *
                     FROM tl_page
                     WHERE type='root'
                     AND (dns=? OR fallback='1' AND languageRoot=?)
                     AND language=?"
                )
                    ->execute(
                    $strSearchDNS,
                ...
                if ($objPage->next()) {
                    ...
                    $_SESSION['TL_ERROR'][] = sprintf(
                        $GLOBALS['TL_LANG']['tl_website_builder_multilang']['missing_root'],
                        $strDNS,
                        $strSource
                    );
                }
            ...
            while ($objPage->next()) {
                if ($objPage->fallback) {
                    if (!$objPage->languageRoot) {
                        $strDNS = $objPage->dns ? $objPage->dns : '-';

                        $arrPages[$strDNS] = array(
                ...
                if ($arrPage['fallback']) {
                    if (isset($arrPages[$arrPage['languageRoot']])) {
                        ...
                        if (!isset($arrRef['pages'][$arrPage['language']])) {
                            ...
                            $_SESSION['TL_ERROR'][] = sprintf(
                                $GLOBALS['TL_LANG']['tl_website_builder_multilang']['duplicate_language'],
                                $arrPage['languageRoot'],
                                $arrPage['language'],
                                $arrRef['pages'][$arrPage['language']]['id'],
                                $arrPage['id']
                            );
                        }
                    }
                    ...
                    $strDNS = $arrPage['dns'] ? $arrPage['dns'] : '-';

                    if (isset($arrPages[$strDNS])
                    ) {
                        $arrRef = & $arrPages[$strDNS];
                        if (!isset($arrRef['pages'][$arrPage['language']])) {
                            $arrRef['pages'][$arrPage['language']] = $arrPage;
                        }
                        else {
                        ...
                        $_SESSION['TL_ERROR'][] = sprintf(
                            $GLOBALS['TL_LANG']['tl_website_builder_multilang']['missing_fallback'],
                            $strDNS,
                            $arrPage['id']
                        );
                    }
        ...
        while ($objPage->next()) {
            if ($objPage->type == 'root') {
                ...
                if ($arrStructure[$objPage->languageMain]['languages'][$objPage->language] == false) {
                    ...
                    $_SESSION['TL_ERROR'][] = sprintf(
                        $GLOBALS['TL_LANG']['tl_website_builder_multilang']['duplicate_page_language'],
                        $objPage->title,
                        $objPage->alias,
                        $objPage->id,
                        $arrStructure[$objPage->languageMain]['languages'][$objPage->language]['title'],
                        $arrStructure[$objPage->languageMain]['languages'][$objPage->language]['alias'],
                        $arrStructure[$objPage->languageMain]['languages'][$objPage->language]['id'],
                        $objPage->language
                    );
                }
            }
    ...
    protected function cloneStructure($objPage, $objParent, $strLang, &$arrMapping)
    {
        ...
        $objNewPage = $this->Database
            ->prepare("SELECT * FROM tl_page WHERE id=?")
            ->execute($intId);
        $objChild   = $this->Database
